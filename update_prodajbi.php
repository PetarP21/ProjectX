...
<div class="w3-top">
  <div class="w3-bar w3-red w3-card w3-left-align w3-large">
    ...
    <a href="update_prodajbi.php" class="w3-bar-item w3-button w3-padding-large w3-white">Продажби</a>
...
<div class="w3-row-padding w3-padding-64 w3-container">
  <div class="w3-content">
    ...
    ID на клиент: <input type="text" name="idklient"><br>
    ID на служител: <input type="text" name="idslujitel"><br>
    Дата на продажба: <input type="text" name="data"><br>
    ...
    <p class="w3-text-grey"><?php
        ...
        if (isset($_POST["submit"])) {
            ...
            $idslujitel = $_POST["idslujitel"];
            $data = $_POST["data"];
            $sql ="UPDATE Prodajbi set Client_id='$idklient',Slujitel_id='$idslujitel',Data_p='$data' where Prodajba_id='$idprodajba'";
        ...
        echo "Таблица с клиенти";
        if ($result->num_rows > 0) {
            echo "<table style='width:100%' border=1>
        <tr>
            <th class='w3-container w3-red w3-center'>ID</th>
            <th class='w3-container w3-red w3-center'>Име</th> 
            <th class='w3-container w3-red w3-center'>Адрес</th> 
            <th class='w3-container w3-red w3-center'>Телефон</th> 
        </tr>";
        while ($row = $result->fetch_assoc()) {
            echo "<tr><th>" . $row["Client_id"] . "</th><th>" . $row["Ime_k"] . "</th><th>" . $row["Adres_k"] . "</th><th>" . $row["Telefon_k"] ."</th></tr>";
        }echo "</table><br>";
        }
        $sql = "SELECT * FROM Slujitel";
        $result = $conn->query($sql);
        echo "Таблица със служители";
        if ($result->num_rows > 0) {
            echo "<table style='width:100%' border=1>
        <tr>
            <th class='w3-container w3-red w3-center'>ID</th>
